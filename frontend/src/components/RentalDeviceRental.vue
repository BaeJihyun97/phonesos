<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            DeviceRental # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            DeviceRental
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <Number label="RentalDeviceId" v-model="value.rentalDeviceId" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/deviceRentals'), this.value)
                        } else {
