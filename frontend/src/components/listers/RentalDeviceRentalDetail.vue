<template>
    <v-card outlined>
        <v-card-title>
            DeviceRental # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Number label="RentalDeviceId" v-model="item.rentalDeviceId" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/deviceRentals/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
